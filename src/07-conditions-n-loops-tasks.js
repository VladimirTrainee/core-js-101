...
function getFizzBuzz(num) {
  ...
  if (num % 3 === 0 || num % 5 === 0) {
    for (let i = 1; i >= 0; i -= 1) {
      for (let j = 1; j >= 0; j -= 1) {
        const res = (3 ** i) * (5 ** j);
        if ((i > 0 || j > 0) && num % res === 0) {
          str = 'Fizz'.repeat(i) + 'Buzz'.repeat(j);
          break;
        }
      }
      if (str !== '') break;
...
function doRectanglesOverlap(rect1, rect2) {
  const inF = (x1, x2, xx1, xx2) => {
    const negative = (x1 < xx1 && x1 < xx2 && x2 < xx1 && x2 < xx2)
      || (x1 > xx1 && x1 > xx2 && x2 > xx1 && x2 > xx2);
    return !negative;
  };

  return inF(rect1.left, rect1.left + rect1.width, rect2.left, rect2.left + rect2.width)
    && inF(rect1.top, rect1.top + rect1.height, rect2.top, rect2.top + rect2.height);
}
...
function isInsideCircle(circle, point) {
  const width = point.x - circle.center.x;
  const height = point.y - circle.center.y;
  return (width ** 2 + height ** 2) < circle.radius ** 2;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const val = str.substring(0, i) + str.substring(i + 1);
